...
def test_analyze_project_uses_openai_compatible_when_configured(monkeypatch) -> None:
    _settings(monkeypatch, api_key="test-key", base_url="https://example.test/v1", model="test-model")
    monkeypatch.setattr(
        llm,
...
def test_generate_screenplay_falls_back_when_model_output_is_invalid(monkeypatch) -> None:
    _settings(monkeypatch, api_key="test-key", base_url="https://example.test/v1", model="test-model")
    monkeypatch.setattr(llm, "_call_openai_compatible", lambda prompt: {"bad": "shape"})
...
def test_call_openai_compatible_parses_json_object(monkeypatch) -> None:
    _settings(monkeypatch, api_key="test-key", base_url="https://example.test/v1", model="test-model")
    ...
    assert llm._call_openai_compatible("prompt") == {"characters": [], "locations": []}
